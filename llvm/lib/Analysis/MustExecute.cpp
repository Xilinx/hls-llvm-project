...
bool LoopSafetyInfo::allLoopPathsLeadToBlock(const Loop *CurLoop,
  ...
  auto KeyP = std::make_pair(BB, CurLoop);
  if (MustExecuteInLoopMap.find(KeyP) == MustExecuteInLoopMap.end()) {
    ...
    for (const auto *Pred : Predecessors) {
      ...
      if (blockMayThrow(Pred)) {
        MustExecuteInLoopMap.insert({KeyP, false});
        return false;
      }
    ...
    MustExecuteInLoopMap.insert({KeyP, true});
    return true;
  }

  return MustExecuteInLoopMap.find(KeyP)->second;
}

void LoopSafetyInfo::clear() {
  MustExecuteInLoopMap.clear();
}
...
bool SimpleLoopSafetyInfo::isGuaranteedToExecute(const Instruction &Inst,
  ...
  if (HLS && ConservativeCheck) {
    // Currently, HLS utilizes for-loop form to stop some optimizations. Thus,
    // do not analyze for loop form even we could when ConservativeCheck is
    // enabled.
    // TODO: Remove the ConservativeCheck when proper check is added in those
    //       optmizations.
  ...
  // NOTE: Below check is more precise, and could enable more LICM. However, it
  //       will fail dataflow checker on case:
  //       UnitSuite/Dataflow/interface_and_channel/StableIO/dataflow/test7
  // If the instruction is in the header block for the loop (which is very
  // common), it is always guaranteed to dominate the exit blocks.  Since this
  // is a common case, and can save some work, check it now.
  if (Inst.getParent() == CurLoop->getHeader())
...
bool MustBeExecutedContextPrinter::runOnModule(Module &M) {
  ...
  GetterTy<LoopInfo> LIGetter = [&](const Function &F) {
    DTs.push_back(make_unique<DominatorTree>(const_cast<Function &>(F)));
    LIs.push_back(make_unique<LoopInfo>(*DTs.back()));
    return LIs.back().get();
  };
  ...
  GetterTy<PostDominatorTree> PDTGetter = [&](const Function &F) {
    PDTs.push_back(
        make_unique<PostDominatorTree>(const_cast<Function &>(F)));
...
public:
  ...
  void printInfoComment(const Value &V, formatted_raw_ostream &OS) override {
    ...
    OS << Loops[0]->getHeader()->getName();
    for (size_t i = 1; i < Loops.size(); ++i)
      OS << ", " << Loops[i]->getHeader()->getName();
    OS << ")";
  }
};
} // namespace
...
bool llvm::mayContainIrreducibleControl(const Function &F, const LoopInfo *LI) {
  ...
  return containsIrreducibleCFG<const BasicBlock *, RPOTraversal,
...
MustBeExecutedContextExplorer::findForwardJoinPoint(const BasicBlock *InitBB) {
  ...
  DEBUG(dbgs() << "\tFind forward join point for " << InitBB->getName()
                    << (LI ? " [LI]" : "") << (PDT ? " [PDT]" : ""));
  ...
  DEBUG(dbgs() << "\t\t#Worklist: " << Worklist.size() << "\n");
  ...
  if (PDT)
    if (const auto *InitNode = PDT->getNode(const_cast<BasicBlock *>(InitBB)))
  ...
  DEBUG(dbgs() << "\t\tJoin block candidate: " << JoinBB->getName() << "\n");
  ...
  DEBUG(dbgs() << "\tJoin block: " << JoinBB->getName() << "\n");
  return JoinBB;
}
const BasicBlock *
MustBeExecutedContextExplorer::findBackwardJoinPoint(const BasicBlock *InitBB) {
  ...
  DEBUG(dbgs() << "\tFind backward join point for " << InitBB->getName()
                    << (LI ? " [LI]" : "") << (DT ? " [DT]" : ""));
  ...
  // conditionals only.
  if (DT)
    if (const auto *InitNode = DT->getNode(const_cast<BasicBlock *>(InitBB)))
      if (const auto *IDomNode = InitNode->getIDom())
...
MustBeExecutedContextExplorer::getMustBeExecutedNextInstruction(
  ...
  auto PPTI = dyn_cast<TerminatorInst>(PP);
  if (!PPTI) {
    const Instruction *NextPP = PP->getNextNode();
    DEBUG(dbgs() << "\tIntermediate instruction does transfer control\n");
    return NextPP;
  }
  ...
  DEBUG(dbgs() << "\tNo join point found\n");
  return nullptr;
}
...
MustBeExecutedContextExplorer::getMustBeExecutedPrevInstruction(
  ...
  if (!ExploreInterBlock && IsFirst) {
    DEBUG(dbgs() << "\tReached block front in intra-block mode, done\n");
    return nullptr;
  }
