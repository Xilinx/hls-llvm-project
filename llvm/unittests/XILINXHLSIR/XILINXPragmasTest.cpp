...
protected:
  void SetUp() override {
    ...
    SubF = Function::Create(FTy, Function::InternalLinkage, "", M.get());
...
TEST_F(HLSIRBuilderTest, Allocation) {
  HLSIRBuilder IB(BB);

  // Check if succeed to create alloction PragmaInst to SubF.
  Value *LAV = IB.CreateXlxFunctionAllocationInst(SubF, StringRef("function"),
                                                  3);
  EXPECT_TRUE(isa<XlxFunctionAllocationInst>(LAV));
