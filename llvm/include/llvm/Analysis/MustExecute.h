...
class LoopSafetyInfo {
  ...
  // Used to cache if the BB must execute in the loop.
  using MustExecuteInLoopMapTy =
      DenseMap<std::pair<const BasicBlock *, const Loop *>, bool>;
  MustExecuteInLoopMapTy MustExecuteInLoopMap;
...
public:
  ...
  /// Clears MustExecuteInLoopMap cache.
  void clear();
  ...
  virtual bool isGuaranteedToExecute(const Instruction &Inst,
                                     const DominatorTree *DT,
                                     const Loop *CurLoop,
                                     bool ConservativeCheck = false) = 0;
...
public:
  ...
  bool isGuaranteedToExecute(const Instruction &Inst,
                             const DominatorTree *DT,
                             const Loop *CurLoop,
                             bool ConservativeCheck = false) override;
};
