...
// for FE
public:
    static PlatformBasic* getInstance();
...
// FIXME, for FE to support old code, will be delete in future
    ...
    bool isValidAXILiteMemType (const std::string& AXILiteMemType) const;
    bool isValidAXILiteMemImpl (const std::string& AXILiteMemImpl) const;
    bool isValidAXILiteMemLatency (const unsigned AXILiteMemLatency) const;
    std::string getDefaultAXILiteMemType() const;
    std::string getDefaultAXILiteMemImpl() const;
    unsigned getDefaultAXILiteMemLatency() const;
...
private:
    bool verifyOpIsSupported (OP_TYPE op) const;
    bool verifyMemImplIsSupported (MEMORY_IMPL memImpl) const;
...
private:
    ...
    // for loading
    bool loadCoreBasic(const std::string& libraryName);
    void createCoreBasics(std::string coreName, std::string op, std::string impl, int maxLat, int minLat, bool isPublic);
    ...
    std::map<IMPL_TYPE, std::string> mImplEnum2Str;
    std::map<std::string, IMPL_TYPE> mImplStr2Enum;
